...
(function($) {
    ...
    $(window).on('load', function() {
        $('[data-pages="fe-header"]').each(function() {
            var $header = $(this)
            $header.header($header.data())
        })
    });

    $('[data-pages="fe-header-toggle"]').on('click touchstart', function(e) {
        e.preventDefault();
        var el = $(this)

        var header = el.attr('data-pages-element');
        $('body').toggleClass('menu-opened');
        $('[data-pages="fe-header-toggle"]').toggleClass('on');

    });
    $(window).on("resize", function() {
        $('[data-pages="fe-header"]').header('updateAffix');
    })
    $(window).on("scroll", function() {
        var ScrollTop = parseInt($(window).scrollTop());
        if (ScrollTop > 1) {
            $('[data-pages="fe-header"]').header('addMinimized');
        } else {
            if (ScrollTop < 10) {
                $('[data-pages="fe-header"]').header('removeMinized');
            }
        }
    });
